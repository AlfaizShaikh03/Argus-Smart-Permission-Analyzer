...
@Singleton
class PerformManualScanUseCase @Inject constructor(
    ...
    suspend fun execute(): Result<List<AppEntity>> {
        return try {
            println("PerformManualScanUseCase: Starting comprehensive scan...")
            ...
            scanResult.fold(
                onSuccess = { newApps ->
                    println("PerformManualScanUseCase: Scan found ${newApps.size} apps")
                    ...
                    println("PerformManualScanUseCase: Merged ${mergedApps.size} apps with user feedback")
                ...
                onFailure = { error ->
                    println("PerformManualScanUseCase: Scan failed - ${error.message}")
                    Result.failure(error)
                }
            )
        } catch (e: Exception) {
            println("PerformManualScanUseCase: Exception during scan - ${e.message}")
            Result.failure(e)
        }
    }
        ...
        return newApps.map { newApp ->
            ...
            if (feedback != null) {
                ...
                if (permissionsChanged) {
                    ...
                    println("PerformManualScanUseCase: ${newApp.appName} permissions changed, recalculating with user feedback")
                    ...
                    println("PerformManualScanUseCase: ${newApp.appName} permissions unchanged, preserving user feedback")
                ...
                println("PerformManualScanUseCase: ${newApp.appName} no user feedback, using fresh scores")
                newApp
            }
        }
    }
    ...
    private fun loadUserFeedback(): Map<String, UserFeedback> {
        return try {
            ...
            if (feedbackData.isNotEmpty()) {
                ...
                println("PerformManualScanUseCase: Loaded ${userFeedbackMap.size} user feedback entries")
            }

            userFeedbackMap
        } catch (e: Exception) {
            println("PerformManualScanUseCase: Error loading user feedback: ${e.message}")
            emptyMap()
        }
    }
